fn main() {
    ...
    let mut friend_table: Vec<Vec<bool>> = vec![vec![false]; n * m];

    for _m in 0..m {
        for _n in 0..n {
            let s: u8 = get_char_from_vec_vec(_m, _n, m, n, &a);
            set_friend_table(n as usize, _n, s as usize, &a[m], &mut friend_table);
        }
    }

    println!("{:?}", friend_table);
}
...
fn set_friend_table(
    ...
    if n == 0 {
        table[person_n][line[1] as usize] = true;
    } else if n == n_max - 1 {
        table[person_n][line[n_max - 2] as usize] = true;
    } else {
        for i in 1..n_max - 2 {
            table[person_n][line[i - 1] as usize] = true;
            table[person_n][line[i + 1] as usize] = true;
        }
    }
